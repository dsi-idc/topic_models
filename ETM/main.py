...
105
--load_from
results/etm_wikipedia_K_50_Htheta_800_Optim_adam_Clip_0.0_ThetaAct_relu_Lr_0.005_Bsz_1000_RhoSize_300_trainEmbeddings_1
...
0
--emb_path
C:\\Users\\avrahami\\Documents\\Private\\IDC\\influencer_influenced_project\\topic_models\\embeddings_model\\tweets_cbow_300
--epochs
20
--load_from
results/etm_wikipedia_K_50_Htheta_800_Optim_adam_Clip_0.0_ThetaAct_relu_Lr_0.005_Bsz_1000_RhoSize_300_trainEmbeddings_1
...
if not args.train_embeddings:
    ...
    # not sure where next line is being used... maybe delete it...
    #vect_path = os.path.join(args.data_path.split('/')[0], 'embeddings.pkl')
    mime = mimetypes.guess_type(emb_path)
    ...
    # in case the file type is not text - we'll assume it is a Gensim model, and we'll treat it as such
        ...
        emb_model = gensim.models.Word2Vec.load(emb_path)
        embeddings = np.zeros((vocab_size, args.emb_size))
        words_found = 0
        for i, word in enumerate(vocab):
            try:
...
if args.mode == 'train':
    ...
    val_ppl = evaluate(model, 'val')
    pd.DataFrame({'val_ppls': all_val_ppls, 'train_loss': all_train_loss}).to_csv("results_table.csv")
else:
    print(os.getcwd())
    with open(ckpt, 'rb') as f:
        model = torch.load(f)
    ...
    with torch.no_grad():
        ...
        # now prnting all in English, using GoogleTranslate
